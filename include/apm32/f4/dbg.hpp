...
class probe {
private:
  static inline std::array<std::unique_ptr<gpio::output_pin>, 16> pins_{};
  gpio::output_pin* const pin_;
  probe_mode const mode_;
public:
  static void init_channel(probe_channel ch, gpio::port prt, gpio::pin pn) {
    pins_[std::to_underlying(ch)] = std::make_unique<gpio::output_pin>(
