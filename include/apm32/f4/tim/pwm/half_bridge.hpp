...
struct half_bridge_config {
  base_config pwm;
  std::array<output_pin, LegCount> hi_pins;
  std::array<output_pin, LegCount> lo_pins;
  std::optional<break_pin> bk_pin;
};

namespace detail {

void configure_base(peripheral_registers* regs, base_config const& conf);

[[nodiscard]] std::unique_ptr<gpio::alternate_pin> configure_break_input(
    peripheral_registers* regs,
    std::optional<break_pin> const& bk_pin
);
...
[[nodiscard]] std::unique_ptr<gpio::alternate_pin>
configure_output(peripheral_registers* regs, output_pin const& pin);
...
private:
  peripheral_registers* const regs_ = timer::regs;
  nvic::irq_number const update_irqn_ = timer::update_irqn;
  nvic::irq_number const break_irqn_ = timer::break_irqn;
  std::array<uint32_t volatile*, 4> const compare_regs_ = timer::ccr_regs;

  emb::units::hz_f32 freq_;
  emb::chrono::nanoseconds_i32 deadtime_;

  std::array<std::unique_ptr<gpio::alternate_pin>, LegCount> hi_pins_;
  std::array<std::unique_ptr<gpio::alternate_pin>, LegCount> lo_pins_;
  std::unique_ptr<gpio::alternate_pin> bk_pin_;
public:
  half_bridge(half_bridge_config<LegCount> conf) {
    ...
    detail::configure_base(regs_, conf.pwm);

    bk_pin_ = detail::configure_break_input(regs_, conf.bk_pin);
    detail::configure_bdt(regs_, conf.pwm, conf.bk_pin);

    for (auto i = 0u; i < LegCount; ++i) {
      detail::configure_channel(regs_, static_cast<channel>(i));
      hi_pins_[i] = detail::configure_output(regs_, conf.hi_pins[i]);
      lo_pins_[i] = detail::configure_output(regs_, conf.lo_pins[i]);
    }
  ...
  count_direction count_direction() const {
  ...
  dutycycle_type dutycycle() const {
    ...
    float const reload_val = regs_->AUTORLD;
