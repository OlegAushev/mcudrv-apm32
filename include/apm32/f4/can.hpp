...
enum class peripheral_id : uint32_t {
  can1,
  can2
};
...
public:
  ...
  peripheral_id id() const { return id_; }

  peripheral_registers* reg() { return reg_; }

  void start();
  void stop();

  bool mailbox_full() const {
    if ((reg_->TXSTS_B.TXMEFLG0 + reg_->TXSTS_B.TXMEFLG1 +
         reg_->TXSTS_B.TXMEFLG2) == 0) {
      return true;
    }
    return false;
  }
  ...
  uint32_t rxfifo_level(rx_fifo fifo) const {
    switch (fifo) {
    case rx_fifo::fifo0:
      return reg_->RXF0_B.FMNUM0;
    case rx_fifo::fifo1:
      return reg_->RXF1_B.FMNUM1;
    }
    return 0;
  }
...
public:
  ...
  void set_interrupts_priority(nvic::IrqPriority rx0_priority,
