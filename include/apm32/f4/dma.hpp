...
public:
  memory_buffer(stream& s) : stream_(s) {
    ...
    stream_.regs()->M0ADDR = static_cast<uint32_t>(data_.data());
  }
...
public:
  memory_double_buffer(stream& s) : stream_(s), buf0(s), buf1(s) {
    ...
    stream_.regs()->NDATA = uint32_t(Size);
    stream_.regs()->M0ADDR = uint32_t(buf0.data());
    stream_.regs()->M1ADDR = uint32_t(buf1.data());
  }
};
