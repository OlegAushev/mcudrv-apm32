...
class stream : public peripheral<stream, stream_id, stream_count> {
public:
  using peripheral_type = peripheral<stream, stream_id, stream_count>;
private:
  stream_id const id_;
  stream_registers* const reg_;
  static inline std::array<bool, 2> _is_clock_enabled{};
public:
  stream(config conf);

  stream_registers* reg() {
    return reg_;
  }

  void
  configure_interrupts(uint32_t interrupt_bitset, nvic::IrqPriority priority);
...
public:
  memory_buffer(stream& s) : stream_(s) {
    stream_.reg()->SCFG_B.DBM = 0;
    stream_.reg()->NDATA = static_cast<uint32_t>(data_.size());
    stream_.reg()->M0ADDR = static_cast<uint32_t>(data_.data());
  }
...
public:
  memory_double_buffer(stream& s) : stream_(s), buf0(s), buf1(s) {
    stream_.reg()->SCFG_B.DBM = 1;
    stream_.reg()->NDATA = uint32_t(Size);
    stream_.reg()->M0ADDR = uint32_t(buf0.data());
    stream_.reg()->M1ADDR = uint32_t(buf1.data());
  }
};
