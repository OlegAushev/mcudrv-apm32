...
stream::stream(config conf)
    : peripheral_type(conf.stream_id),
      id_(conf.stream_id),
      reg_(detail::streams[std::to_underlying(conf.stream_id)]) {
  enable_clock(id_);
  DMA_Config(reg_, &conf.hal_config);
}
...
void stream::configure_interrupts(
    ...
    nvic::IrqPriority priority
) {
  if ((interrupt_bitset & DMA_INT_FEIFLG) == DMA_INT_FEIFLG) {
    reg_->FCTRL_B.FEIEN = 1;
  }

  if (interrupt_bitset != DMA_INT_FEIFLG) {
    set_bit(reg_->SCFG, interrupt_bitset & 0x1E);
  }
