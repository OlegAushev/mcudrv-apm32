...
void detail::configure_base(
    peripheral_registers* regs,
    base_config const& conf
) {
  core::ensure(conf.prescaler.has_value());

  auto const timebase_freq = core::clock_freq<emb::units::hz_f32>() /
...
[[nodiscard]] std::unique_ptr<gpio::alternate_pin> configure_break_input(
  ...
  return std::make_unique<gpio::alternate_pin>(pinconf);
}
...
std::unique_ptr<gpio::alternate_pin>
detail::configure_output(peripheral_registers* regs, output_pin const& pin) {
