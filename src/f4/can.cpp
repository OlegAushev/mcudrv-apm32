...
peripheral::peripheral(
    ...
    : peripheral_type(id),
      id_(id),
      reg_(detail::peripherals[std::to_underlying(id)]),
      rx_pin_(rx_pinconf),
      tx_pin_(tx_pinconf) {
  enable_clock(id);
  CAN_Config(reg_, &conf.hal_config);
}

rxmessage_attr peripheral::register_rxmessage(CAN_FilterConfig_T filter) {
  assert(filter_count_ < max_fitler_count_);
...
exec_status peripheral::put_frame(can_frame const& frame) {
  ...
  uint32_t mailboxid = reg_->TXSTS_B.EMNUM;
  if (mailboxid > 2) {
  ...
  // set up id
  if (frame.id <= 0x7FF) {
    write_reg(reg_->sTxMailBox[mailboxid].TXMID, (frame.id << 21));
  } else if (frame.id <= 0x1FFFFFFF) {
    write_reg(reg_->sTxMailBox[mailboxid].TXMID, (frame.id << 3));
    reg_->sTxMailBox[mailboxid].TXMID_B.IDTYPESEL = 1;
  } else {
    return exec_status::invalid_argument;
  }

  // set up dlc
  reg_->sTxMailBox[mailboxid].TXDLEN_B.DLCODE = frame.len & 0x0F;

  // set up data field
  write_reg(
      reg_->sTxMailBox[mailboxid].TXMDL,
      (uint32_t(frame.payload[0]) << 0) | (uint32_t(frame.payload[1]) << 8) |
          (uint32_t(frame.payload[2]) << 16) |
          (uint32_t(frame.payload[3]) << 24)
  );
  write_reg(
      reg_->sTxMailBox[mailboxid].TXMDH,
  ...
  reg_->sTxMailBox[mailboxid].TXMID_B.TXMREQ = 1;
...
peripheral::get_frame(can_frame& frame, rx_fifo fifo) const {
  ...
  // get id, len, filter
  if (reg_->sRxMailBox[fifo_idx].RXMID_B.IDTYPESEL == 0) {
    frame.id = reg_->sRxMailBox[fifo_idx].RXMID >> 21;
  } else {
    frame.id = reg_->sRxMailBox[fifo_idx].RXMID >> 3;
  }

  frame.len = uint8_t(reg_->sRxMailBox[fifo_idx].RXDLEN_B.DLCODE);

  rxmessage_attr attr{};
  attr.filter_idx = reg_->sRxMailBox[fifo_idx].RXDLEN_B.FMIDX;
  attr.fifo = fifo;

  // get data
  frame.payload[0] = uint8_t(reg_->sRxMailBox[fifo_idx].RXMDL_B.DATABYTE0);
  frame.payload[1] = uint8_t(reg_->sRxMailBox[fifo_idx].RXMDL_B.DATABYTE1);
  frame.payload[2] = uint8_t(reg_->sRxMailBox[fifo_idx].RXMDL_B.DATABYTE2);
  frame.payload[3] = uint8_t(reg_->sRxMailBox[fifo_idx].RXMDL_B.DATABYTE3);
  frame.payload[4] = uint8_t(reg_->sRxMailBox[fifo_idx].RXMDH_B.DATABYTE4);
  frame.payload[5] = uint8_t(reg_->sRxMailBox[fifo_idx].RXMDH_B.DATABYTE5);
  frame.payload[6] = uint8_t(reg_->sRxMailBox[fifo_idx].RXMDH_B.DATABYTE6);
  frame.payload[7] = uint8_t(reg_->sRxMailBox[fifo_idx].RXMDH_B.DATABYTE7);
...
void peripheral::configure_interrupts(uint32_t interrupt_bitset) {
  set_bit(reg_->INTEN, interrupt_bitset);
}

void peripheral::set_interrupts_priority(
    nvic::IrqPriority rx0_priority,
    nvic::IrqPriority rx1_priority,
    nvic::IrqPriority tx_priority
) {
  set_irq_priority(
