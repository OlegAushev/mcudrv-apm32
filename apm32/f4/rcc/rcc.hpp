...
{
  return static_cast<std::uint32_t>(pll_input_frequency()
                                    / clock_config::pll_vco_in);
}
...
constexpr T sysclk_frequency()
{
  ...
  if constexpr (C::sysclk_src == sysclk_src::hse) {
    ...
    static_assert(vco_in >= vco_in_min && vco_in <= vco_in_max,
                  "PLL VCO input frequency out of range");

    constexpr uint64_t vco_out = vco_in * C::pll1a_mult;
    static_assert(vco_out >= vco_out_min && vco_out <= vco_out_max,
                  "PLL VCO output frequency out of range");

    static_assert(vco_out % to_divisor(C::pll1c_div) == 0,
...
static_assert(clock_config::pll1a_mult >= 50 && clock_config::pll1a_mult <= 432,
              "pll1a_mult must be in 50..432");

static_assert(clock_config::plld_div >= 2 && clock_config::plld_div <= 15,
              "plld_div must be in 2..15");

static_assert(hse_value >= hse_min && hse_value <= hse_max,
              "HSE_VALUE out of supported range");

static_assert(sysclk_frequency<std::uint64_t>()
                      % to_divisor(clock_config::ahb_div)
                  == 0,
              "HCLK frequency is not an integer number of Hz");

static_assert(hclk_frequency<std::uint64_t>()
                      % to_divisor(clock_config::apb1_div)
                  == 0,
              "PCLK1 frequency is not an integer number of Hz");

static_assert(hclk_frequency<std::uint64_t>()
                      % to_divisor(clock_config::apb2_div)
                  == 0,
              "PCLK2 frequency is not an integer number of Hz");

static_assert(sysclk_frequency<std::uint64_t>() <= sysclk_max,
              "SYSCLK frequency exceeds maximum");

static_assert(hclk_frequency<std::uint64_t>() <= hclk_max,
              "HCLK frequency exceeds maximum");

static_assert(cpu_frequency<decltype(clock_config::expected_cpu_frequency)>()
                  == clock_config::expected_cpu_frequency,
              "computed CPU frequency does not match expected CPU frequency");

static_assert(pclk1_frequency<std::uint64_t>() <= pclk1_max,
              "PCLK1 frequency exceeds maximum");

static_assert(pclk2_frequency<std::uint64_t>() <= pclk2_max,
