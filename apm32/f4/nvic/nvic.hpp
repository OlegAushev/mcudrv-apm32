...
private:
  ...
  std::uint8_t sub_;
public:
  explicit constexpr irq_priority(std::uint8_t preempt_pri,
                                  std::uint8_t sub_pri = 0)
      : preempt_{
            std::clamp(preempt_pri, std::uint8_t{0}, preempt_priority_max)},
...
template<irq_lock_policy Policy = irq_lock_policy::all,
