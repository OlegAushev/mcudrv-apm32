...
inline const std::array<ADC_T*, peripheral_count> adc_instances = {
    ADC1, ADC2, ADC3};

inline Peripheral to_peripheral(const ADC_T* instance) {
    return static_cast<Peripheral>(std::distance(
            adc_instances.begin(),
            std::find(adc_instances.begin(), adc_instances.end(), instance)));
}
...
public:
    ...
    void init_injected(const PinConfig& pin_config,
                       const InjectedChannelConfig& channel_config);
    void init_regular(const PinConfig& pin_config,
