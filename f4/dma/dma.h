...
public:
    ...
    static Stream* instance(StreamId stream_id) {
        return emb::singleton_array<Stream, stream_count>::instance(
                std::to_underlying(stream_id));
    }

    void init_interrupts(uint32_t interrupt_bitset, IrqPriority priority);
    ...
    void enable() { _stream_reg->SCFG_B.EN = 1; }
protected:
...
public:
    ...
    MemoryDoubleBuffer(Stream& stream)
            : _stream(stream), buf0(stream), buf1(stream) {
