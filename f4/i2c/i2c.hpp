...
enum class Peripheral : unsigned int { i2c1, i2c2, i2c3 };
...
inline const std::array<I2C_T*, peripheral_count> instances = {
    I2C1, I2C2, I2C3};
...
inline constexpr std::array<IRQn_Type, peripheral_count> event_irqn = {
    I2C1_EV_IRQn, I2C2_EV_IRQn, I2C3_EV_IRQn};
inline constexpr std::array<IRQn_Type, peripheral_count> error_irqn = {
...
public:
    ...
    static bool is_event(uint32_t sts_regs, Event event) {
        return (sts_regs & std::to_underlying(event)) ==
...
public:
    void init_event_interrupts(bool enable_buf_it, IrqPriority priority) {
        ...
        set_irq_priority(impl::event_irqn[std::to_underlying(_peripheral)],
                         priority);
    }

    void init_error_interrupts(IrqPriority priority) {
        _reg->CTRL2_B.ERRIEN = 1;
        set_irq_priority(impl::error_irqn[std::to_underlying(_peripheral)],
                         priority);
    }

    void enable_event_interrupts() {
        enable_irq(impl::event_irqn[std::to_underlying(_peripheral)]);
    }
    void enable_error_interrupts() {
        enable_irq(impl::error_irqn[std::to_underlying(_peripheral)]);
    }
    void disable_event_interrupts() {
        disable_irq(impl::event_irqn[std::to_underlying(_peripheral)]);
    }
