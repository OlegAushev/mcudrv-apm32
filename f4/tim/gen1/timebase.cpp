...
TimebaseTimer::TimebaseTimer(Peripheral peripheral, const TimebaseConfig& config)
        : impl::AbstractTimer(peripheral, OpMode::timebase) 
{
    auto cfg = config;

    float timebase_freq = (float(core_clk_freq()) / 2) / float(config.hal_base_config.division+1);
