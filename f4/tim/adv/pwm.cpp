        ...
        : impl::AbstractTimer(peripheral, OpMode::pwm_generation) 
{
    ...
    if (cfg.hal_base_config.period == 0 && cfg.freq != 0) {
        ...
        case TMR_COUNTER_MODE_DOWN:
            cfg.hal_base_config.period = uint32_t((timebase_freq / config.freq) - 1);
            break;
        case TMR_COUNTER_MODE_CENTER_ALIGNED1:
        case TMR_COUNTER_MODE_CENTER_ALIGNED2:
        case TMR_COUNTER_MODE_CENTER_ALIGNED3:
            cfg.hal_base_config.period = uint32_t((timebase_freq / config.freq) / 2);
            break;
        }
    ...
    case TMR_CLOCK_DIV_2:
        _t_dts_ns = 2 * float(config.hal_base_config.division+1) * 1000000000.f / float(core_clk_freq());
        break;
    case TMR_CLOCK_DIV_4:
        _t_dts_ns = 4 * float(config.hal_base_config.division+1) * 1000000000.f / float(core_clk_freq());
        break;
    }
...
void PwmTimer::_initialize_bdt(const PwmConfig& config, BkinPin* pin_bkin) {
    ...
    if (bdt_cfg.deadTime == 0) {
        ...
        if (config.deadtime_ns <= 0X7F * _t_dts_ns) {
            ...
            bdt_cfg.deadTime = uint16_t((config.deadtime_ns - 64 * 2 * _t_dts_ns) / (2 * _t_dts_ns));
            bdt_cfg.deadTime |= 0x80;
        } else if (config.deadtime_ns <= 63 * 8 * _t_dts_ns) {
            bdt_cfg.deadTime = uint16_t((config.deadtime_ns - 32 * 8 * _t_dts_ns) / (8 * _t_dts_ns));
            bdt_cfg.deadTime |= 0xC0;
        } else if (config.deadtime_ns <= 63 * 16 * _t_dts_ns) {
            bdt_cfg.deadTime = uint16_t((config.deadtime_ns - 32 * 16 * _t_dts_ns) / (16 * _t_dts_ns));
