...
void mcu::tests::chrono_test() {
    ...
    mcu::chrono::steady_clock::add_task(taskLedToggle,
                                        std::chrono::milliseconds(100));
    mcu::chrono::steady_clock::register_delayed_task(
            []() { bsp::nucleo::led_red.toggle(); },
    ...
    for (auto i = 1; i <= 4; ++i) {
        ...
        if (i % 2 != 0) {
            EMB_ASSERT_EQUAL(bsp::nucleo::led_green.read(),
                             mcu::gpio::pin_state::active);
        } else {
            EMB_ASSERT_EQUAL(bsp::nucleo::led_green.read(),
        ...
        if ((i < 2) || (i == 4)) {
            EMB_ASSERT_EQUAL(bsp::nucleo::led_red.read(),
                             mcu::gpio::pin_state::inactive);
        } else {
            EMB_ASSERT_EQUAL(bsp::nucleo::led_red.read(),
                             mcu::gpio::pin_state::active);
